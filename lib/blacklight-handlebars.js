...
module.exports.registerBlacklightHelpers=function(registry, translationMethod){
	...
	HB.registerHelper("component", function(model, resourceType,  options){
		...
		if(model){
			...
			if(type){
				...
				global.bl.logger.get("blacklight-core.handlebars").error("No _sling_resourceType in model: ", model);
	...
	HB.registerHelper('translate', function (key, options) {
		...
		var site=_.get(meta,["blacklight","site"])
		var staticTranslation = _.get(global.bl, ["staticTranslation","sites",site]);

		if(!staticTranslation){throw new Error("No staticTranslation method provided for site '" + site + "'. Please include bl-plugin-translate-static or some other implementation plugin via site.js");}
		return  new HB.SafeString(staticTranslation(key, options));
	});
	...
	HB.registerHelper('img-opt', function (path, options) {
		...
		if(!meta.blacklight.mode){
			throw new Error("img-opt error: you must provide a correct 'mode' indicator (author/publish) in @root.meta.blacklight.mode");
		}
		...
		if(path && path.match && path.match(/^\/content\/dam\//)){
			return global.bl.publicMount + "img-opt/" + meta.blacklight.mode + path + "/" + quality + size + ".jpg";
		}else{
