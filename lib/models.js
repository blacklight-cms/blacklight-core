...
var log=global.bl.logger.get("blacklight-cms.models");
 ...
 * models.generateModelFromRaw(mySlingContent, myTypeRegistry, externalUtilities)
 *   .then(function(model){
 ...
 * @param {Object} externalUtilities - A dictionary of useful app-specific values and functions which will be made available to each model processor method
 * @returns {Promise} A promise which is thenable, and will fulfill with the completed data model.
 */
exports.generateModelFromRaw = function(slingPageContent, typeRegistry, externalUtilities, skipModelProcessors){
	...
	externalUtilities.page = externalUtilities.page || {};
	if(externalUtilities.isTopLevel){
		externalUtilities.page.root=result;
		delete externalUtilities.isTopLevel;
	}
	...
	function rawToModel(content, target){
		...
		_.forEach(content,function(val,key){
			...
			if(isComponent(val)){
				target[targetKey]=makeBlankComponent();
				if(val._meta && val._meta._sling_resourceType){
					/// This is to deal with the edge case where the incoming sling has already stashed the resource type under _meta
					target[targetKey]._meta=val._meta;
					Object.defineProperty(val, "_meta", {enumerable: false});
				}
				if(!skipModelProcessors){
	...
	function processNonComponent(content, target, targetKey){
		...
		if(targetKey[0]=="_" && targetKey!=="_meta"){
			if(!target.hasOwnProperty("_meta")) {target._meta={}; log.warning("Uh-oh. model.js~processNonComponent had to add _meta property");}
			target=target._meta;
		}
	...
	function processModel(content, model){
		...
		return function(resolve,reject){
			...
			if(!model || !model._meta){ resolve(); return; } // In case the model was deleted by slingPReprocess, since being queued up

			var cmp = typeRegistry.get(resourceType);
			
			model._meta.utilities = externalUtilities;
			Object.defineProperty(model._meta, "utilities", {enumerable: false});
				...
				if(cmp.model){
					if(_.isFunction(cmp.model.process)){
						cmp.model.process(model, _.defaults(
							...
							blacklight:{publicRoot: typeRegistry.publicRoot, emailError: typeRegistry.emailError},
							_:_}, 						
							externalUtilities));
...
exports.makeBlankComponent = makeBlankComponent;
 ...
 * @private
 */
function makeBlankComponent(resourceType){
	var obj = {_meta:{}}
	Object.defineProperty(obj, "_meta", {
		enumerable: false
	});
	if(resourceType)obj._meta._sling_resourceType=resourceType;
	return obj;
}
