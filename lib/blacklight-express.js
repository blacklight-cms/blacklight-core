...
module.exports = function(options){
	...
	var registry = componentTypes.createRegistry({componentPaths: options.componentPaths, componentCacheDisable:options.componentCacheDisable,  componentCacheClearOnChange: options.componentCacheClearOnChange, postProcessors: options.postProcessors, postProcessOptions: options.postProcessOptions});
	...
	return function(req, res, next){
		...
		favicon(req, res, function onNext(){
			...
			var slingConnector = slingConnectors.ports[req.socket.localPort];
			if(!slingConnector) throw(new Error("No sling connector matching request port: " + req.socket.localPort));
				
			var requestedMode = req.headers["x-sling-source"];
			if(requestedMode && (requestedMode!=slingConnector.runMode)){
				slingConnector=slingConnectors.modes[requestedMode];
			...
			if(extension && !_.contains(supportedExtensions, extension)){
				...
				slingConnector.getPipable(req.path, req.method, headers).pipe(res);
			}
			else{
				slingConnector.get(path.slingPath,"infinity",function(err, slingData, slingResponseObject){
					if(err){
						...
						renderError(res, err, extension, "Sling connection error on request: " + req.path);
						...
						models.generateModelFromRaw(slingData, registry, utilities)
							.then(function(model){
								...
								renderError(res, error, extension, "Unhandled exception (possibly an error in framework code?)")
								throw(error);
							});
					}
