...
module.exports = function(options){
	...
	return function(req, res, next){
		favicon(req, res, function onNext(){
			...
			req.requestPath = path.requestPath;
...
function parsePath(path){
	...
	var requestPath = (parts.length?"/":"") + parts.join("/") + (name?"/":"") + name; 
	var slingPath = (parts.length?"/":"") + parts.join("/") + (name?"/":"") + name + "/_jcr_content";

	return {original: path, slingPath: slingPath, selectors: selectors, extension: ext, requestPath: requestPath}
}
