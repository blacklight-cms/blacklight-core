...
module.exports = function(options){
	...
	var logger=global.bl.logger.get("blacklight-core.express");
	...
	if(options.requestPreprocessors){
		...
		_.each(this.requestPreprocessors, function(rp){
			...
			if(rp.redirect){slingRedirects.push(rp);}
			if(rp.slingPreprocess){slingPreProcessors.push(rp);}
			if(rp.proxy){ proxies.push(rp); }
		});
	}
	...
	return function(req, res, next){
		...
		if(req.path==="/favicon.ico"){
			...
			while(!found && i<slingRedirects.length){
				...
				found=curRedirect.path.test(path);
				if(found){
			...
			var slingConnector = req.bl.sling;
			var requestedMode = req.bl.mode;
			...
			if(extension && !_.includes(supportedExtensions, extension)){
				...
				if(!foundProxy)
					slingConnector.get(path.slingPath, "infinity", function(err, slingData){
						if(err){
							...
							err.slingPath = path.slingPath;
							err.slingConnectMessage = slingData;
							renderError(req, res, err, extension, "Sling connection error on request:\n<br><span style='font-size:16px'>" + slingConnector.baseUri.replace(/\/$/,"") + "/" + path.slingPath.replace(/^\//,"") + ".infinity.json</span>");
				...
				function processSling(slingData){
						...
						slingData.meta = slingData.meta || {_sling_resourceType:"blacklight/meta"};

						slingData.meta.blacklight = slingData.meta.blacklight || {};
						slingData.meta.blacklight.mode = requestedMode;
						slingData.meta.blacklight.site = req.bl.site;
						slingData.meta.blacklight.public_mount = global.bl.publicMount;
						slingData.meta.blacklight.apps_mount = global.bl.appsMount;
						if(requestedMode==="author"){slingData.meta.blacklight.author=true;}	
						slingData.meta.blacklight.sling_proxy = req.headers["x-proxy-type"];
						...
						while(i<slingPreProcessors.length){
							...
							if( ( curSlingProcessor.path && curSlingProcessor.path.test(path.originalPath) ) || ( curSlingProcessor.type && curSlingProcessor.type.test(slingData._sling_resourceType) ) ){
								if(curSlingProcessor.slingPreprocess){allCurSlingProcessors.push(curSlingProcessor.slingPreprocess);}
							}
							i++;
						}	
						...
						function renderModelWithTemplate(model){
							try{
								...
								var body = registry.render(model.data, {data:{root:model.data, contextPath:"."}, topLevel:true}, path.selectors, path.extension, req.method, req);
								if(!res.headersSent){
	...
	function renderError(req, res, err, extension, context){
		...
		if(err.stack){
			stack=err.stack.toString().replace(/\\n/g,"\n");
			delete err.stack;
		}
		...
		if(err.slingConnectMessage){
			statusCode = err.slingConnectMessage.statusCode || statusCode;
			delete err.slingConnectMessage;
		}

		// TODO: add other 404 and 500 status logging options
		if(emailError){
			if(statusCode==404){
				if(req.headers.referer){ 
					emailError({subject: "Blacklight 404 error", text: err, req: req});
				}
			}else{
				emailError({subject: "Blacklight 500 error", text: err, req: req, stack:stack});
			}
		}
		...
		if(extension=="html"){
			...
			if(res.headersSent){
				logger.error("Couldn't set status code for page with error:", req.method, req.path)
			}else{
				res.status(statusCode);
			}
			res.send("<div style='color:#911; font-family:sans-serif'><h1>" + context + "</h1><h3>" + err.message + "</h3></div><hr><pre>" + JSON.stringify(err,null,"  ") + "<hr>" + stack + "</pre>");
		}else{
			res.status(statusCode).json({message: context, error: err, stack: err.stack});
		}
	}
