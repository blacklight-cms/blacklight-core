...
module.exports = function(options){
	...
	var slingPostProcessors=[], slingRedirects=[], proxies=[];
	...
	if(options.pathHandlers){
		this.pathHandlers= _.isArray(options.pathHandlers) ? options.pathHandlers : [options.pathHandlers];

		_.each(this.pathHandlers, function(ph){
			if(_.isString(ph.path)) ph.path = new RegExp(ph.path);
			ph.redirect && slingRedirects.push(ph);
			ph.postProcess && slingPostProcessors.push(ph);
			ph.proxy && proxies.push(ph);
		});
	}
	...
	return function(req, res, next){
		...
		favicon(req, res, function onNext(){
			...
			if(extension && !_.contains(supportedExtensions, extension)){
				...
				while(!foundProxy && i<proxies.length){
					if(proxies[i].path.test(path.trimmedPath)){
						foundProxy=true;
				...
				if(!foundProxy)
					slingConnector.get(path.slingPath, "infinity", function(err, slingData){
						if(err){
							err.slingPath = path.slingPath;
							err.slingConnectMessage = slingData;
							renderError(req, res, err, extension, "Sling connection error on request: " + req.path);
				...
				function processSling(slingData){
						...
						var utilities=_.defaults({
								...
								blacklight:{publicRoot: registry.publicRoot, emailError: registry.emailError},
								isTopLevel:true
							}, 
							options.utilities);
						var found=false,i=0;
						...
						while(!found && i<slingPostProcessors.length){
							var curSlingProcessor=slingPostProcessors[i];

							if(found=curSlingProcessor.path.test(path.trimmedPath)){
								try{
									curSlingProcessor.postProcess(slingData, utilities);
								}catch(err){
