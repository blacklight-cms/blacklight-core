 ...
 * @param {function} [options.postProcessOptions] 
 * @param {function} [options.postProcessOptions.beautifyHTML] - Set to true to re-indent HTML before output 
 */
exports.createRegistry = function(options){
	...
	// Add default framework-level components 
	componentRoots.push(_path.resolve(__dirname + "/../components"));

	_.each(componentRoots,function(componentRoot){
		console.log("Mounting component path: " + componentRoot);
	})
	...
	_.each(componentRoots, function(componentRoot){
		...
		if(!options.componentCacheDisable && options.componentCacheClearOnChange){
			console.log("Watching: " + componentRoot);
