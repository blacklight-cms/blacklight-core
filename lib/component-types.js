...
var HB=require("./blacklight-handlebars");
...
exports.createRegistry = function(options){
	...
	HB.registerBlacklightHelpers(registry);
    ...
    //setup array of component type paths
	var componentTypeRoots = !_.isArray(options.componentPaths)?[options.componentPaths]:options.componentPaths;

    //add built-in components path
    componentTypeRoots.push(_path.resolve(__dirname + "/../components"));

    console.log("All component paths: " + componentTypeRoots.toString());
    //monitor each of the paths for changes
    _.each(componentTypeRoots, function(componentTypeRoot){
	...
	function ComponentType(partialPath){
			...
			if(tmpl && tmpl.name && this.templates[tmpl.name] && this.templates[tmpl.name].fn){
				...
				try{
					if(extension=="json" || extension == "xml"){
						...
						return renderRaw(model, registry);
				}catch(err){
					throw(err)
				}
			}
        ...
        //figure out if any of the component paths have
        //this component
		var componentPath;
		var pathsTried = [];

        _.each(componentTypeRoots, function(componentTypeRoot){
            //only check if we haven't already found a valid path
            if(!componentPath){
                var pathToTest = _path.join(componentTypeRoot, partialPath);
                if(fs.existsSync(pathToTest)){
                    //path found, use it
                    componentPath = this.path = pathToTest;
                }else{
                    pathsTried.push(pathToTest);
                }
		...
		if(!componentPath){
			this.error="Component type path not found: " + pathsTried.toString();
			return;
		};
	...
	registry.render = function(model, selectors, extension, httpMethod){
		...
		if(rtype){
			...
			console.log("registry.render: Model has no _meta._sling_resourceType: " + JSON.stringify(model) );
		}
	}
