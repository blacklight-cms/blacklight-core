...
var HB;
var log=global.bl.logger.get("blacklight-core.components");
var events = require('events');

global.bl.componentWatch = global.bl.componentWatch || new events.EventEmitter();
...
exports.createRegistry = function(options){
	...
	_.each(componentRoots, function(componentRoot){
		...
		if(!fs.existsSync(componentRoot)){
			log.error("Specified component root does not exist:", componentRoot);
			return;
		}

		log.info("Mounting:", componentRoot);

		if(!options.componentCacheDisable && options.componentCacheClearOnChange){
			var chokidar=require("chokidar");
			log.info("Watching:", componentRoot);
			chokidar.watch(componentRoot).on("all", function(e,f){
				var now = new Date();
				if((now-lastWatchEvent)>500){
					lastWatchEvent=new Date();
					log.info("Changed component source, clearing caches: ", e, f);
					cache={};
					global.bl.componentWatch.emit("change", f);
				}
			});
		}
	 });
	...
	function ComponentType(partialPath){
		...
		this.render=function(model, templateOptions, selectors, extension, httpMethod, req){
			var resultText;
			...
			if(tmpl && tmpl.name && this.templates[tmpl.name] && this.templates[tmpl.name].fn){
				try{
					if(!model._meta){model._meta={}; Object.defineProperty(model, "_meta", {enumerable: false});}
					model._meta.req = req; // We place the req object here, for use by handlebars helpers
				...
				try{
					if(extension=="json" || extension == "xml"){
						if(_.includes(selectors, "meta")){
							exports.setMetaEnumeration(model, true, !_.includes(selectors, "all"));
						}
						if(extension=="json")
	...
	registry.render = function(model, templateOptions, selectors, extension, httpMethod, req){
		var rtype = getResourceType(model);			

		if(rtype){
			var componentType = registry.get(rtype);
			return componentType.render(model, templateOptions, selectors, extension, httpMethod, req);
		}else{
...
exports.findBestTemplateMatch=function(files, requiredBasename, requestSelectors, requestExtension, httpMethod){
	...
	/// TODO:  what about "request/file.txt.bar.html" ... wouldn't .txt always win, if present?
 	if(requestExtension!="html"){
		candidateFiles = _.filter(candidateFiles, function(file){
			return _.includes(file.selectors, requestExtension);
		});		
	}
...
function whittleBySelector(files, selector){
	var workingFiles = _.filter(files, function(file){
		return _.includes(file.selectors, selector);
	});
