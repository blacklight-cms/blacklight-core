...
module.exports=function(routerSettings){
	...
	var router=function(req, res, next){
		var processor = getProcessorsByPath(req, res);
		if(!processor){ next(); return;}

		var $={
			page: processor.page,
			express: {req:req,res:res,next:next},
		...
		processor.modelProcessor.process(data, $, function(err, options){
			// render precendence: options, modelProcessor.render(), processor.template
			if(processor.page.extension==="json"){
				res.json(data);
			}else{
				if(processor.template){
					res.header("content-type", "text/html").end(processor.template(data));
				}else{
					res.json(data);
				}
			}
	...
	var getProcessorsByPath=function(req){
		...
		var action=base.slice(routePath.length);
